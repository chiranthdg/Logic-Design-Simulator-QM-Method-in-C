...
void swap(NODE,NODE);           //data of two nodes is swapped
void disp();            //various column with pairings are displayed
void further_groupings();           //the minterms are paired
void end_loop(NODE);            //the extra node in a list is deleted
void display_implicants();              //the implicants are displayed
void implicants(NODE);          //initializes each term as an implicant
...
int main()
{
    ...
    save->link=NULL;           //storing null value in link field of list storing prime implicants
...
void swap(NODE temp1,NODE temp2)  //swapping all the data of two nodes
{
    int x,y,i=0;
    i=var;
    ...
    while(temp!=NULL)
    {
        ...
        while(next!=NULL)
        {
            ...
            if(count==1) //checks if the two terms differ by one place in binary notation
            {
                ...
                printf("\n");
                p=p->link=getnode();           // one extra node that is to be deleted
            }
            next=next->link;
        }
        temp=temp->link;
